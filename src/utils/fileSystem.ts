...
import { STANDARDS, PHOTO05_DTD } from '../constants/standards';
import { WORK_MASTER } from '../constants/workMaster';
import { dateFromXml, toSerial, buildPhotoXml } from './xmlBuilder';
...
/** PHOTOフォルダハンドルを取得 */
export const getPhotoDir = async (rootHandle: FileSystemDirectoryHandle, std: Standard, create = false): Promise<FileSystemDirectoryHandle> => {
...
export const buildXmlMapFromNodes = (nodes: HTMLCollectionOf<Element>): Record<string, Partial<Photo>> => {
...
export const readPhotoXmlNodes = async (photoDir: FileSystemDirectoryHandle): Promise<{ decoded: string; nodes: HTMLCollectionOf<Element> }> => {
...
export const saveXmlToDir = async (photos: Photo[], rootHandle: FileSystemDirectoryHandle, stdId: string): Promise<void> => {
  const std = STANDARDS[stdId];
  const photoDir = await getPhotoDir(rootHandle, std, true);
  const xmlStr = buildPhotoXml(photos, stdId);
  const sjis = new Uint8Array(
    Encoding.convert(Encoding.stringToCode(xmlStr), 'SJIS', 'UNICODE')
  );
...
export const saveDtdToFolder = async (photoDir: FileSystemDirectoryHandle, stdId: string): Promise<void> => {
  const std = STANDARDS[stdId];
  const sjis = new Uint8Array(
...
export const executeChunked = async <T>(tasks: T[], action: (task: T) => Promise<void>): Promise<void> => {
