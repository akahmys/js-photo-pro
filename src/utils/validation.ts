 ...
 *                         着手前及び完成も△（条件付き）
 */
export const validatePhoto = (p: Partial<Photo> & { title?: string; shootingDate?: string; category?: string; workType?: string; discipline?: string; type?: string; subdivision?: string; referenceFileName?: string; referenceTitle?: string }): string[] => {
  const errs: string[] = [];
