...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden animate-fadeIn bg-slate-100">
      ...
      <div className="status-bar no-print flex items-center justify-between bg-slate-900 border-t border-slate-800 text-[10px] text-slate-400 font-bold px-6 py-2">
        <div className="flex items-center gap-6">
          <span className="flex items-center gap-1.5 text-emerald-400">
            <Ic k="check" size={12} /> JS基準適合
          </span>
          <span className="flex items-center gap-1.5">
            <Ic k="hardDrive" size={12} /> {p.rootHandle?.name}
          </span>
          <span className="flex items-center gap-1.5">
            <Ic k="camera" size={12} /> {p.photos.length}枚
          </span>
          {p.errorCount > 0 && (
            <span className="flex items-center gap-1.5 text-orange-400">
              <Ic k="alertCircle" size={12} /> 未完了 {p.errorCount}件
            </span>
          )}
        </div>
      ...
      <ImagePreviewModal previewPhoto={p.previewPhoto} onClose={() => p.setPreviewPhoto(null)} />
