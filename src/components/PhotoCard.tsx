  ...
  return (
    ...
    >
      <div className="aspect-[4/3] relative overflow-hidden bg-slate-100 rounded-t-xl">
        <LazyImage file={p.file} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
        ...
        <div className="absolute top-2 right-2 flex gap-1 z-10">
          {p.referenceFileName && (
            <div className="w-5 h-5 bg-purple-600 rounded-md flex items-center justify-center shadow-md" title="参考図あり">
              <Ic k="list" size={10} cls="text-white" />
            </div>
          )}
          {p.isRepresentative && (
            <div className="w-5 h-5 bg-blue-600 rounded-md flex items-center justify-center shadow-md" title="代表写真">
              <Ic k="star" size={10} cls="text-white" fill="white" />
            </div>
          )}
          {p.isFrequency && (
            <div className="w-5 h-5 bg-emerald-600 rounded-md flex items-center justify-center shadow-md" title="提出頻度">
      ...
      <div className="p-3 flex-1 flex flex-col justify-between bg-white">
        <div>
          ...
          <p className={`text-xs font-bold line-clamp-2 leading-snug ${!p.title ? 'text-orange-500 italic' : 'text-slate-700'}`}>
            {p.title || "（タイトル未入力）"}
          </p>
        </div>
